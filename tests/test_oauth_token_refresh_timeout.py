...
class TestOAuthTokenRefreshTimeout:
    ...
    def test_token_cache_isolation_on_timeout(self, oauth_manager, mock_asyncio_wait_for):
        """Test that token cache remains isolated when timeout occurs."""
        # Pre-populate cache
    ...
    async def test_async_context_cleanup(self, oauth_manager, mock_asyncio_wait_for):
        """Test proper async context cleanup on timeout."""
        mock_asyncio_wait_for.side_effect = asyncio.TimeoutError()
    ...
    def test_multiple_timeout_scenarios(self, oauth_manager, mock_asyncio_wait_for):
        """Test multiple timeout scenarios in sequence."""
        scenarios = [
            asyncio.TimeoutError(),
            asyncio.TimeoutError(),
            {"access_token": "finally_success"}
        ]
        
        mock_asyncio_wait_for.side_effect = scenarios
        ...
        result = asyncio.run(oauth_manager.refresh_token("test_token"))
        assert result["access_token"] == "finally_success"